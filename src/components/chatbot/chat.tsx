...
const Temp = () => {
  ...
  const inputIndustry = useSelector(
    (state: RootState) => state.chatbot.industry
  );
  const inputSelectedRole = useSelector(
    (state: RootState) => state.chatbot.selectedRole
  );
  const inputChatbotName = useSelector(
    (state: RootState) => state.chatbot.chatbotName
  );
  const inputOrganisationName = useSelector(
    (state: RootState) => state.chatbot.organisationName
  );
  ...
  const inputWebsiteURL = useSelector(
    (state: RootState) => state.chatbot.websiteURL
  );
  // const [inputWebsiteURL, setInputWebsiteURL] = useState(useSelector((state: RootState) => state.chatbot.websiteURL));
  const inputIndustryFromNav = useSelector(
    (state: RootState) => state.inputIndustryFromNav.value
  );
  ...
  }, [isSubmittingFromCard, inputText]);
  ...
  const handleSubmit = async (event?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (uploadingEmbeddings) {
        ...
        const response = await ChatBackend(
          inputText,
          sessionId,
          selectedIndustry,
          selectedRole,
  ...
  const handleCardClick: React.MouseEventHandler<HTMLDivElement> = async (
    event
  ) => {
  ...
  const getMessage = () => {
    ...
    if (inputIndustryFromNav) {
      setTitleAndRole(
        `Welcome to Blueberry AI Chatbot specially designed for the ${inputIndustryFromNav} industry`
      );
    } else if (
      inputIndustry &&
      inputSelectedRole &&
      ...
      setTitleAndRole(
        `Introducing ${inputChatbotName}, your dedicated virtual assistant designed specifically for the ${inputIndustry} industry. ${inputChatbotName} is specialised for assistance in ${inputSelectedRole} and is here to enhance your experience and streamline your interactions.`
      );
      ...
      // console.log("Running getMessage else");
      setTitleAndRole("Welcome to Blueberry AI Chatbot");
    }
  };
  ...
  //write function update the session id on every render
      ...
      {isMobile ? (
        <>
          <div className="flex flex-col justify-between mt-20">
            <button
              onClick={toggleForm}
              className="ml-2 mr-2 px-20 py-2 items-center justify-center mb-10 rounded-md border border-black bg-white text-black text-sm hover:shadow-[4px_4px_0px_0px_rgba(0,0,0)] transition duration-200"
            >
      ...
      <div className="chatbot-component flex flex-col w-4/5 px-36">
        ...
        <div
          ...
          className={`flex-1 overflow-y-auto  ${
            showCards ? "flex items-center justify-center" : "items-start chat"
          }`}
        >
        ...
        {/* Query Input */}
        <div className="input-form sticky bottom-2 bg-white p-2 mx-20 border border-gray-300 rounded-full shadow-sm justify-center items-center">
